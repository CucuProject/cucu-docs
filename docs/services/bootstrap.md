...
- A top-level dependency or seeder failure aborts the process with exit code `1`; logs identify run id, stage, status, and error class without serializing the underlying error. The Desktop validation tooling persists a redacted log-derived per-step/per-tenant report and a separate authoritative Mongo totals report; mutation counts per step are still log signals, not authoritative persisted results.
...
- Docker Desktop runtime evidence covers preserved-failure recovery, deadline and owner-absent failures, warning-free ACME/Globex/Initech runs, and duplicate-free reruns. Initech v1/v2 keeps 16 tenant-owned databases, 36 collections, and 2,973 documents identical. Explicit Jobs must retain the established `cucu-bootstrap` release identity so the NetworkPolicy selects them; a differently named release fails dependency readiness before seed. Orion is not a fixture on this branch, and authoritative created/updated/skipped results per step remain tracked by CUC-251/CUC-430.
